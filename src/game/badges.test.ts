...
test("badges unlock once and award their circuit points", () => {
  const badges = newlyEarnedBadges([], {
    ...
    career: { hunts: 0, bouts: 5, molts: 1, stripped: 0, clutches: 0, perfectMolts: 0, worldTitles: 0 },
    wins: 5,
  ...
  assert.deepEqual(newlyEarnedBadges(badges.map((badge) => badge.id), {
    spiders: [spider], career: { hunts: 0, bouts: 0, molts: 0, stripped: 0, clutches: 0, perfectMolts: 0, worldTitles: 0 }, wins: 0, seen: [],
  }), []);
});
...
test("a World Stick title unlocks once after carrying the circuit into a new year", () => {
  const badges = newlyEarnedBadges([], {
    spiders: [spider],
    career: { hunts: 0, bouts: 0, molts: 0, stripped: 0, clutches: 0, perfectMolts: 0, worldTitles: 1 },
    wins: 0,
    seen: [],
  });
